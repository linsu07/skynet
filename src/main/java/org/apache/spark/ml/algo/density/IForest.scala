...
class IForestModel (
    ...
    with IForestParams with MLWritable with AntiSpamAlgorithm {
...
//  require(_trees.nonEmpty, "IForestModel requires at least 1 tree.")
  ...
  override def setConfig(c: Config): IForestModel.this.type = {
    super.setConfig(c)
...
//      .setSummary(trainingSummary)
  ...
  private[ml] def setSummary(summary: Option[IForestSummary]): this.type = {
  ...
  /**
    ...
    * @param data Input data which is a dataset with n_samples rows. This dataset must have a
  ...
  override def transform(data: Dataset[_]): DataFrame = {
    val dataset = addFeatureColumn(data.asInstanceOf[DataFrame],${inputCols})
    transformSchema(data.schema, logging = true)
//    val numSamples = data.count()
    val possibleMaxSamples =$(numOfSamples)
//      if ($(maxSamples) > 1.0) $(maxSamples) else ($(maxSamples) * numSamples)
    val bcastModel = data.sparkSession.sparkContext.broadcast(this)
    // calculate anomaly score
    val normFactor = avgLength(possibleMaxSamples)
    val scoreUDF = udf { (features: Vector) => {
      val avgPathLength = {
        var len  = bcastModel.value.calAvgPathLength(features)
        //(len*10).round
        len
      }
      //avgPathLength
    ...
    val scoreDataset = dataset.withColumn($(outputCols)(0), scoreUDF(col($(featuresCol))))
    scoreDataset.stat.approxQuantile($(outputCols)(0),Array(1 - $(contamination)), $(approxQuantileRelativeError))
    scoreDataset
//    if (threshold < 0) {
...
//    }).sum / trees.length
    var id = 0
    val avgPathLength = trees.map(ifNode => {
      var len = calPathLength(features, ifNode, 0)
//      println(s"id ${id}, length  = ${length}")
      id = id + 1
      val split = 5
      if(len<=2)
        len = 0
  ...
  private def calPathLength(features: Vector,
      ifNode: IFNode,
      currentPathLength: Int): Double ={
//      println("----------------------------------------------------------------")
//      println(s"currentPathLength is ${currentPathLength}")
      ifNode match {
        case leafNode: IFLeafNode =>
//          println(s"next is leaf node, avgLength is ${leafNode.numInstance}")
          currentPathLength + avgLength(leafNode.numInstance)
        case internalNode: IFInternalNode =>
//          println(s"total length of this internalNode is ${internalNode.getTotalLength()}")
          val attrIndex = internalNode.featureIndex
//          println(s"this feature Index is ${attrIndex}, splitValue is ${internalNode.featureValue}")
//          println(s"current sample feature ${attrIndex} is ${features(attrIndex)}")

          if (features(attrIndex) < internalNode.featureValue) {
//            println("less than go left")
            calPathLength(features, internalNode.leftChild, currentPathLength + 1)
          } else {
//            println("more than go right")
...
object IForestModel extends MLReadable[IForestModel] {
  ...
  def main(args: Array[String]): Unit = {
    val model = new IForestModel("abc",Array())
    println(model.avgLength(256))

    val score = Math.pow(2, -1 / 10.24)
    print(score)
  }
}
...
class IForest (
    ...
    with IForestParams with AntiSpamAlgorithm with DefaultParamsWritable{
  ...
  setDefault(
    ...
    contamination -> 0.1,
    bootstrap -> false,
    seed -> this.uid.hashCode.toLong, // linsu change to uid's hashcode
    approxQuantileRelativeError -> 0d,
    outputCols->Array("IForestAnomalyScore"),
    featuresCol->"iforest_features"
  )
...
//  def setAnomalyScoreCol(value: String): this.type = set(anomalyScoreCol, value)
  ...
  /**
    ...
    */
  private[ml] def splitData(dataset: Dataset[_]): RDD[(Int, Array[Vector])] = {
    ...
    set(numOfSamples,possibleMaxSampels)
    ...
    val rddPerTree = {
      ...
      }.groupBy(_._1).mapValues(x => x.map(_._2)).map {  //(rowIndex,[treeid1,treeid2,...])
    ...
    }  // rdd  行数是treenumber， 列是 array[vector]， array长度是possibleMaxSampels
    .map{ case (treeId:Int,samples:Array[Vector])=>
      val shuffled = samples.map(_.toArray)
        .transpose.map(arr=>shuffle(arr))
        .transpose
        .map {
        arr =>
          val ret = arr.map{v=>
            if(v>4.0) (Random.nextDouble() - 0.5) * 1 + 4.0
            else
              v
          }
          Vectors.dense(ret)
  ...
  /**
    ...
    */
  override def fit(data: Dataset[_]): IForestModel = {
    val dataset = addFeatureColumn(data.asInstanceOf[DataFrame],${inputCols})
...
//    model.setSummary(Some(summary))
  ...
  private[ml] def sampleFeatures(
    ...
    if (subFeatures == numFeatures) {
      (data.toArray.map(vector =>
        vector.asInstanceOf[DenseVector].values)
      ...
      val featureIdx = {
        ...
        for(i<-0 to (numFeatures-1))
          buf.append(i)
        random.shuffle(buf).take(subFeatures)
//        random.shuffle(0 to (numFeatures - 1))
      }
  ...
  /**
    ...
    * @return tree's root node
    */
  private[ml] def iTree(data: Array[Array[Double]],
...
trait IForestParams extends Params {
  ...
  final val numOfSamples :IntParam =
    new IntParam(this, "numOfSamples", "the number of samples to " +
    "draw from data to train each tree.", ParamValidators.gt(0))
...
//    SchemaUtils.appendColumn(schema, $(predictionCol), DoubleType)
  ...
  def addFeatureColumn(dataset:DataFrame,inputColNames:Array[String]):DataFrame={
    val addFeature = udf{
      (row:Row)=>

        //val features = row.toSeq.map(_.asInstanceOf[String].toDouble).toArray
        val features = row.toSeq.map(value => {
        ...
        val into: Vector = Vectors.dense(features)
        into
    }
    dataset.asInstanceOf[DataFrame].withColumn(${featuresCol},addFeature(struct(inputColNames.map(col):_*)))
  }
}
