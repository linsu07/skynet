...
trait DiscreteFeatureAddTFParam extends Params {
...
class DiscreteFeatureAddTF(override val uid: String) extends AntiSpamModel[DiscreteFeatureAddTF] with AntiSpamAlgorithm with AggregateByKey with DiscreteFeatureAddTFParam{
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    var data1 = dataset.withColumns(outputs,getAddColumnsFunction(${inputCols}.toList,addDiscreteFeature))

    if (${MaxValue})
    {
      val outputs1 = ${outputCols}.map(info=> info+"Max").toSeq
      data1 = data1.withColumns(outputs1,getAddColumnsFunction(${inputCols}.toList,addNumValue,"max"))
    }
    if (${MinValue})
    {
      val outputs = ${outputCols}.map(info=> info+"Min").toSeq
      data1 = data1.withColumns(outputs,getAddColumnsFunction(${inputCols}.toList,addNumValue,"min"))
    }

    if (${MeanValue})
    {
      val outputs = ${outputCols}.map(info=> info+"Mean").toSeq
      data1 = data1.withColumns(outputs,getAddColumnsFunction(${inputCols}.toList,addNumValue,"mean"))
    }

    if (${DevValue})
    {
      val outputs = ${outputCols}.map(info=> info+"Dev").toSeq
      data1 = data1.withColumns(outputs,getAddColumnsFunction(${inputCols}.toList,addNumValue,"Dev"))
    }
    data1
    }
